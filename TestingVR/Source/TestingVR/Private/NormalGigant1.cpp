...
ANormalGigant1::ANormalGigant1()
{
    ...
    GetCharacterMovement()->MaxWalkSpeed = 630.0f;
...
void ANormalGigant1::IgnoreChannel()
{
    //공격안할떄는 Ignore으로 처리
    RightArmCollision->SetRelativeLocation(FVector(0.0f, 0.0f, 0.39f));
    RightArmCollision->SetWorldScale3D(FVector(0.27f, 0.79f, 0.25f));
...
void ANormalGigant1::CancelIgnoreChannel()
{
...
void ANormalGigant1::move()
{
    ...
    if (bIsAttackAnimationPlaying == false)
    {
        ...
        if (bIsPlayerInSight() && DistanceAiToCharacter.Length() > GigantAttackDistance)
        {
            //플레이어한테 다가오고
            MoveToPlayer();
        }
    ...
    if (bIsGigantDie)
    {
        ...
        if (AnimInstance && !bIsOnceDieMT)
        {
            ...
            if (SelectedMontage)
            {
                ...
                GetWorldTimerManager().SetTimer(TimerHandle_Die1Destroy, this, &ANormalGigant1::DieDestroy, 15.0f, false);
            }
        }
    }
